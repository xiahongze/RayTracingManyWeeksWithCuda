...
__device__ ray camera::get_ray(int i, int j, int s_i, int s_j, float recip_sqrt_spp, curandState *local_rand_state) const
{
    // Get a randomly-sampled camera ray for the pixel at location i,j, originating from
    // the camera defocus disk.

    auto pixel_center = pixel00_loc + (i * pixel_delta_u) + (j * pixel_delta_v);
    auto pixel_sample = pixel_center + pixel_sample_square(s_i, s_j, recip_sqrt_spp, local_rand_state);
...
__device__ vec3 camera::pixel_sample_square(int s_i, int s_j, float recip_sqrt_spp, curandState *local_rand_state) const
{
    // Returns a random point in the square surrounding a pixel at the origin.
    auto px = -0.5 + recip_sqrt_spp * (s_i + curand_uniform(local_rand_state));
    auto py = -0.5 + recip_sqrt_spp * (s_j + curand_uniform(local_rand_state));
